window.addEventListener('DOMContentLoaded', () => {
    ...
    if (btnPlay) {
        btnPlay.addEventListener('click', () => {
            ...
            officeImage.src = 'Assets/images/office.jpg';
            officeImage.alt = 'Office';
            officeImage.style.width = '100vw';
            officeImage.style.height = '100vh';
            officeImage.style.objectFit = 'contain';
            officeImage.style.cursor = 'default';
            officeImage.draggable = false;

            const fitImageToViewport = () => {
                if (!officeImage.naturalWidth || !officeImage.naturalHeight) {
                    return;
                }

                const viewportRatio = window.innerWidth / window.innerHeight;
                const imageRatio = officeImage.naturalWidth / officeImage.naturalHeight;

                if (viewportRatio > imageRatio) {
                    officeImage.style.width = `${window.innerHeight * imageRatio}px`;
                    officeImage.style.height = `${window.innerHeight}px`;
                } else {
                    officeImage.style.width = `${window.innerWidth}px`;
                    officeImage.style.height = `${window.innerWidth / imageRatio}px`;
                }
            };

            officeImage.addEventListener('load', fitImageToViewport, { once: true });
